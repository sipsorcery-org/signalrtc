...
namespace signalrtc
{
    public struct JanusResult
    {
        public bool Success;
        public string Error;

        public JanusResult(bool result, string error)
        {
            Success = result;
            Error = error;
        }
    ...
    public class JanusRestClient
    {
        ...
        /// session.
        /// </summary>
        public async Task<JanusResult> StartSession()
        {
            _sessionID = await CreateSession(_ct).ConfigureAwait(false);

            if(_sessionID == 0)
            {
                return new JanusResult(false, "Janus session creation failed.");
            }
            else
            {
                _ = Task.Factory.StartNew(StartLongPoll, TaskCreationOptions.LongRunning);
                return new JanusResult(true, null);
            }
        }
        ...
        public async Task<JanusResult> StartEcho(string sdpOffer)
        {
            var pluginID = await AttachPlugin(JanusPlugins.ECHO_TEST).ConfigureAwait(false);

            if (pluginID == 0)
            {
                return new JanusResult(false, "Janus failed to create echo plugin.");
            }
            else
            {
                var resp = await StartEcho(pluginID, sdpOffer).ConfigureAwait(false);
                
                if (resp.error?.reason != null)
                {
                    return new JanusResult(false, resp.error.reason);
                }
                else
                {
                    return new JanusResult(true, null);
        ...
        private async Task<ulong> CreateSession(CancellationToken ct)
        {
            ...
            _logger.LogDebug($"Janus CreateSession Result={sessResp.Data.janus}.");
        ...
        private async Task StartLongPoll()
        {
            ...
            _logger.LogDebug($"Long poll thread exiting for session {_sessionID}.");
        ...
        private async Task<JanusResponse> StartEcho(ulong echoPluginID, string offer)
        {
            ...
            if (offerResp.Data.JanusOp == JanusOperationsEnum.error)
            {
                ...
                _logger.LogWarning($"Error starting echo test, code={errResp.error.code}, reason={errResp.error.reason}.");
            }
            else
            {
